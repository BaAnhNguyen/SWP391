...
const AssignBloodUnits = () => {
    ...
    const fetchRequestDetails = async () => {
        try {
            ...
            try {
                const singleResponse = await fetch(`${API_BASE_URL}/needrequest/${requestId}`, {
                ...
                console.log("Single request fetch failed, trying all requests:", singleErr);
            }
    ...
    const fetchAvailableBloodUnits = async () => {
        ...
        try {
            ...
            console.log("Blood type charcode analysis:", bloodType ? Array.from(bloodType).map(c => `${c}:${c.charCodeAt(0)}`).join(', ') : 'none');
            ...
            const normalizeBloodType = (type) => {
                ...
                if (type.match(/^(A|B|AB|O)POS$/i)) return type.substring(0, type.length - 3) + "+";
                if (type.match(/^(A|B|AB|O)NEG$/i)) return type.substring(0, type.length - 3) + "-";

                // Handle type with a space like "A +"
                if (type.match(/^(A|B|AB|O)\s+(\+|\-)$/i)) return type.replace(/\s+/, "");
            ...
            console.log(`Making request to: ${API_BASE_URL}/bloodunit/filter/for-request?componentType=${componentType}&bloodType=${encodeURIComponent(normalizedBloodType)}`);

            // Try with the standard blood types directly if normalization might be failing
            const standardBloodTypes = ["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"];
            ...
            try {
                response = await fetch(
                    `${API_BASE_URL}/bloodunit/filter/for-request?componentType=${componentType}&bloodType=${encodeURIComponent(normalizedBloodType)}`,
                    {
                        headers: { Authorization: `Bearer ${token}` },
                    }
                );
                ...
                if (response.ok) {
                    ...
                    console.log("Request failed with normalized blood type, status:", response.status);
            ...
            if (!matchFound) {
                ...
                exactMatch = standardBloodTypes.find(type =>
                    type.toLowerCase() === bloodType.trim().toLowerCase() ||
                    type.toLowerCase().replace(/[+-]/, "") === bloodType.trim().toLowerCase());
            ...
            if (!matchFound || !response.ok) {
                ...
                try {
                    ...
                    if (errorData.message === "Invalid or unsupported blood type.") {
                        errorMessage = `${t("needRequest.invalidBloodType")}: ${normalizedBloodType}. ${t("needRequest.supportedTypes")}: ${errorData.supportedTypes?.join(", ")}`;
                    } else {
                        errorMessage = errorData.message || t("needRequest.fetchBloodUnitsError");
                    }
                } catch (err) {
                ...
                console.log("Attempting fallback: Getting all blood units and filtering locally");
                try {
                    ...
                    if (allUnitsResponse.ok) {
                        ...
                        console.log(`Got ${allUnits.length} total blood units for manual filtering`);
                        ...
                        for (const [stdType, _] of Object.entries(manualCompatibility)) {
                            if (upperBloodType.includes(stdType) ||
                        ...
                        if (!recipientType) {
                            for (const stdType of Object.keys(manualCompatibility)) {
                                if (upperBloodType.startsWith(stdType[0])) {
                                    ...
                                    const isPositive = upperBloodType.includes("+") ||
                                        upperBloodType.includes("POS") ||
                                        upperBloodType.includes("POSITIVE");
                                    const isNegative = upperBloodType.includes("-") ||
                        ...
                        if (!recipientType) {
                            recipientType = Object.keys(manualCompatibility).find(t => t === upperBloodType) || "A+";
                            console.log("Defaulting to blood type:", recipientType);
                        }

                        const compatibleTypes = manualCompatibility[recipientType] || [];
                        console.log("Using manual compatibility for", recipientType, ":", compatibleTypes);
                        ...
                        console.log(`Manually filtered: ${filteredUnits.length} compatible units found`);
            ...
            console.log("Successfully made API request with blood type:", exactMatch || normalizedBloodType);
            ...
            const availableUnits = compatibleUnits.filter(unit =>
                new Date(unit.DateExpired) > new Date()
            );
    ...
    const getFilteredAndSortedUnits = () => {
        ...
        if (filterComponent !== "all") {
            filtered = filtered.filter(unit => unit.ComponentType === filterComponent);
        }
        ...
        <div className="assign-blood-units-container">
            ...
            {requestDetails && (
                <div className="request-details-card">
                    ...
                    {requestDetails.originalBloodGroup && requestDetails.originalBloodGroup !== requestDetails.bloodGroup && (
                    ...
                    <div className="request-info-grid">
                        <div className="info-item">
                            ...
                            <span className={`blood-badge blood-${requestDetails.bloodGroup?.replace('+', 'pos').replace('-', 'neg')}`}>
                        ...
                        <div className="info-item">
                            ...
                            <span className={`urgency-badge urgency-${requestDetails.urgency?.toLowerCase()}`}>
            ...
            <div className="selection-summary">
                <div className="summary-item">
                    <span className="summary-label">{t("needRequest.selectedUnits")}:</span>
                    <span className="summary-value">{selectedBloodUnits.length}</span>
                </div>
                <div className="summary-item">
                    <span className="summary-label">{t("needRequest.requiredUnits")}:</span>
                    <span className="summary-value">{requestDetails?.units || 0}</span>
                </div>
                <div className="summary-item">
                    <span className="summary-label">{t("needRequest.remainingUnits")}:</span>
                    <span className="summary-value">
                        {Math.max(0, (requestDetails?.units || 0) - selectedBloodUnits.length)}
                    </span>
                </div>
            </div>
            ...
            <div className="blood-units-section">
                <h3>{t("needRequest.compatibleBloodUnits")} ({filteredUnits.length})</h3>
                    ...
                    <div className="table-container">
                        <table className="blood-units-table">
                            ...
                            <tbody>
                                {filteredUnits.map((unit) => {
                                    ...
                                    const maxUnitsReached = selectedBloodUnits.length >= (requestDetails?.units || 0);
                      ...
                      `.trim()}
                                        ...
                                        >
                                            ...
                                            <td>
                                                <span className={`blood-badge blood-${unit.BloodType.replace('+', 'pos').replace('-', 'neg')}`}>
                                                    {unit.BloodType}
                                                </span>
                                            </td>
                                            ...
                                            <td>
                                                <span className={`source-badge source-${unit.SourceType || 'unknown'}`}>
                                                    {unit.SourceType || 'Unknown'}
                                                </span>
                                            </td>
                                            <td>{unit.donorName || '-'}</td>
                                            <td className="note-cell" title={unit.note || ''}>
                                                {unit.note ? (unit.note.length > 30 ? `${unit.note.substring(0, 30)}...` : unit.note) : '-'}
                                            </td>
                                        </tr>
                                    );
                                })}
            ...
            <div className="assignment-form">
                <div className="form-group">
                    <label htmlFor="appointmentDate">{t("needRequest.appointmentDate")}:</label>
                    <input
                        type="datetime-local"
                        id="appointmentDate"
                ...
                <div className="form-actions">
                    ...
                    >
                        {assigning ? t("common.processing") : t("needRequest.confirmAssignment")}
                    </button>
                    <button
                        onClick={() => navigate(-1)}
