...
const DonateRequestForm = ({ onRequestCreated }) => {
  ...
  const handleNext = async (e) => {
    ...
    try {
      ...
      console.log("Đang lấy danh sách câu hỏi y tế...");
      ...
      setDebugInfo({ apiUrl, responseStatus: response.status, data });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const screening = questions.map((q) => {
        // Kiểm tra xem q.content có tồn tại không
        if (!q.content) {
          console.error("Câu hỏi không có nội dung:", q);
        }
        return {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          const errorText = await response.text();
          console.error("Lỗi không phải JSON:", errorText);
          throw new Error(
