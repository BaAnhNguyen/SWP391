...
exports.delete = async (req, res) => {
  try {
    ...
    if (req.user.role === "Member") {
      if (nr.status !== "Pending" && nr.status !== "Open" && nr.status !== "Completed") {
        return res
          .status(400)
          .json({ message: "Cannot delete: request is in progress. Status must be Pending, Open, or Completed to delete." });
      }
...
exports.confirm = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: err.message || "Failed to complete request" });
  }
};
